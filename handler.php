...
switch ($action) {
    ...
    case 'update_project':
        ...
        $status = 'Planning';
        if (isset($data['status']) && !in_array($data['status'], ['In Development', 'Released', 'GBA Testing'])) {
            $status = $data['status'];
        } else {
            if ($software_released) {
                $status = $use_gba_testing ? 'GBA Testing' : 'Released';
            } else {
        ...
        if ($action === 'add_project') {
            ...
            $stmt->bind_param("ssssssssssiii",
                ...
                null_if_empty($data['description']), null_if_empty($data['ap']),
                null_if_empty($data['cp']), null_if_empty($data['csc']), null_if_empty($data['qb_user']), null_if_empty($data['qb_userdebug']),
                $software_released, $use_gba_testing, $data['project_id']
            );
        }
        $stmt->execute();
        redirect('index.php');
    ...
    // ... sisa case (delete_project, update_status, check_gba_status, dll) tetap sama seperti sebelumnya ...
    case 'delete_project':
    ...
    case 'update_status':
        ...
        if ($stmt->execute()) {
            if ($new_status === 'GBA Testing') {
                ...
                if ($project_data) {
                    $check_task_stmt = $conn->prepare("SELECT id FROM gba_tasks WHERE project_id = ?");
                    $check_task_stmt->bind_param("i", $project_id);
                    ...
                    if (!$existing_task) {
                        ...
                        $task_stmt = $conn->prepare("INSERT INTO gba_tasks (project_id, model_name, ap, cp, csc, qb_user, qb_eng, test_plan_type, progress_status, request_date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
